...
from xdevs.plugins.wrappers.pypdevs import PyPDEVSWrapper
...
class Processor(AtomicDEVS):
    ...
    def extTransition(self, inputs):
        if self.state == "passive":
            self.current_job = inputs[self.i_in][0]
        return "active"
    ...
    def outputFnc(self):
        return {self.o_out: [self.current_job]}
...
class Gpt(Coupled):
    def __init__(self, name, period, obs_time):
        ...
        proc = PyPDEVSWrapper(Processor("processor", 3 * period))
